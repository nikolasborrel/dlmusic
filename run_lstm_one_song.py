...
#### Script to overfit LSTM on one song
model_name = 'one_song'
dataset_path = paths.midi_dir_one_song
...
dataset_split_kwargs = {'p_train': 0.8, 'p_val': 0.2, 'p_test': 0.0,
                        'batch_size': 1,
                        'eval_batch_size': 1}

train, val, test, t = create_dataset_from_midi_one_song(dataset_path, 
                                               lead_instrument, 
                                               accomp_instrument, 
                                               print_info=True,
                                               **tokenizer_kwargs, 
...
#[midi_io.sequence_proto_to_midi_file(seq.to_sequence(), paths.midi_dir_one_song + f'mel{i}.mid') for i, seq in enumerate(train.dataset.inputs)]
#[midi_io.sequence_proto_to_midi_file(seq.to_sequence(), paths.midi_dir_one_song + f'bass{i}.mid') for i, seq in enumerate(train.dataset.targets)]
